<template>
  <div class="list-whats-new">
    <h3>What's new</h3>
    <b-container class="bv-example-row">
      <!-- <b-row>
        <b-col lg="3" class="item-card">2 of 3</b-col>
        <b-col lg="3" class="item-card">3 of 3</b-col>
      </b-row>-->
            <!-- <h3>{{lists}}</h3> -->
      <b-row>
        <div id="items-new" v-for="list in lists" :key="list._id">
          <b-col lg="3" class="item-card">
         <b-card no-body img-alt="Image" img-top class="card-body">

       <img v-if="!list.image" src="https://wanowi.com/public/uploads/products/list/product-default.jpg" alt="user" height="200"
        style="display:block; margin:auto;"/>
           <img v-if="list.image" :src="list.image" alt="productpicture"  width="200"
        height="200"
        style="display:block; margin:auto;"/>
      <!-- <img v-if="product.image" :src="product.image" alt="picture">
      <img v-if="!product.image"
        src="data:image/png;base64,"
        alt="productpicture"
        width="200"
        height="200"
        style="display:block; margin:auto;"
        class="product-image"
      /> -->
      <b-card-body>
        <b-card-title>{{list.productname}}</b-card-title>
        <b-card-sub-title class="mb-2">Total : {{list.qty}}</b-card-sub-title>
        <b-card-text>{{list.price}}</b-card-text>
      </b-card-body>
      <button @click="addtoCart(product._id)" class="button-cart">Add to Cart</button>
    </b-card>
        </b-col>
        </div>
      </b-row>
    </b-container>
    <button>See More Products</button>
  </div>
</template>

<script>
import axios from '../config/axios.js'
export default {
  data () {
    return {
      lists: []
    }
  },
  methods: {
    listNew () {
      axios({
        method: `GET`,
        url: `/products/new`
      })
        .then(({ data }) => {
          console.log(data)
          this.lists = data
        })
        .catch(err => {
          console.log(err)
        })
    }
  },
  created () {
    this.listNew()
  }
}
</script>

<style scoped>
.item-card {
  padding: 3%;
}
.list-whats-new {
  margin-top: 10%;
}

.card-body {
  padding: 0;
  width: 250px;
}

.bv-example-row {
  border: 1px solid grey;
  box-shadow: 0px 10px 10px rgba(0, 0, 0, 0.1);
  transition: all 0.3s ease 0s;
}
</style>
